...
`;
   ...
   window.addToCart = function (id) {
    ...
    fetch("https://grocery-backend-9kla.onrender.com/cart", {
        method: "POST",
        headers: {
...
renderCart();
 ...
 window.checkout = async function () {
    ...
    try {
        const res = await fetch("https://grocery-backend-9kla.onrender.com/create-order", {
            method: "POST",
        ...
        // Agar backend se error aaya (e.g. Razorpay keys missing)
        if (!order.id) {
            console.error("Order Error:", order);
            alert("Backend Error: Order ID nahi mili.");
            return;
        }

        const options = {
            key: "rzp_test_SVoZ2GmFSGHSS5", // 👈 apni asli Key check karein
            amount: order.amount,
            currency: "INR",
            name: "SuperMart",
            ...
            handler: async function (response) {
                ...
                let userEmail = localStorage.getItem("email") || "guest@example.com";
                await fetch("https://grocery-backend-9kla.onrender.com/payment-success", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ email: userEmail, amount: total })
                });
...
window.login = async function(email, password) {
    const res = await fetch("https://grocery-backend-9kla.onrender.com/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
window.signup = async function(name, email, password) {
    const res = await fetch("https://grocery-backend-9kla.onrender.com/signup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
document.getElementById("contact-form").addEventListener("submit", async function(event) {
    ...
    try {
        const res = await fetch("https://grocery-backend-9kla.onrender.com/contact", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
